...
(function () {
  ...
  /* ======================================================================
     15a2 · QUICK PRICE POPUP
     The arrow on a featured card opens a small dialog for THAT project:
     enquire once → its starting price appears (and the shared unlock flag
     opens pricing across the whole site).
     ====================================================================== */
  ...
  function initQuickPrice() {
    ...
    var nameEl = $('#qName'), locEl = $('#qLoc'), body = $('#qBody');
    var lastFocus = null;
    var current = null;

    function fmtAnimate(el, priceStr) {
      var num = parseInt(priceStr.replace(/[^\d]/g, ''), 10);
      if (REDUCED || !num) { el.textContent = priceStr; return; }
      var start = performance.now(), dur = 800, from = Math.round(num * 0.82);
      (function step(now) {
        var t = clamp((now - start) / dur, 0, 1);
        var eased = t === 1 ? 1 : 1 - Math.pow(2, -10 * t);
        el.textContent = 'from US$' + Math.round(from + (num - from) * eased).toLocaleString('en-US');
        if (t < 1) requestAnimationFrame(step);
      })(start);
    }
    ...
    function render() {
      if (priceUnlocked()) {
        body.innerHTML =
          '<p class="qmodal__price"><b id="qVal">from ' + current.price + '</b></p>' +
          '<p class="qmodal__note">Starting price at ' + current.name + '. Our sales team will confirm current availability within one business day.</p>' +
          '<div class="qmodal__cta">' +
          '<a class="btn btn--solid btn--sm" href="' + current.href + '"><span>See availability</span></a>' +
          '<a class="btn btn--ghost btn--ghost-light btn--sm" href="tel:+2202082828"><span>+220 208 2828</span></a>' +
          '</div>';
        return;
      }
      body.innerHTML =
        '<p class="qmodal__price is-locked"><b>US$ ••,•••</b>' +
        '<svg viewBox="0 0 16 16" fill="none" aria-hidden="true"><rect x="3" y="7" width="10" height="7" rx="1.6" stroke="currentColor" stroke-width="1.3"/><path d="M5.5 7V5.2a2.5 2.5 0 0 1 5 0V7" stroke="currentColor" stroke-width="1.3"/></svg></p>' +
        '<p class="qmodal__note">Leave your details once — the starting price appears instantly, for every project.</p>' +
        '<form class="qmodal__form" data-native novalidate>' +
        '<div class="field"><input type="text" id="qf-name" placeholder=" " required autocomplete="name">' +
        '<label for="qf-name">Full name</label><span class="field__line"></span></div>' +
        '<div class="field"><input type="email" id="qf-email" placeholder=" " required autocomplete="email">' +
        '<label for="qf-email">Email address</label><span class="field__line"></span></div>' +
        '<button class="btn btn--solid btn--sm" type="submit"><span>Reveal the price</span></button>' +
        '</form>';
    }

    function open(card) {
      var nameNode = $('.feat-card__name', card);
      var locNode = $('.feat-card__loc', card);
      var priceNode = $('[data-price]', card);
      current = {
        name: nameNode ? nameNode.textContent.trim() : '',
        loc: locNode ? locNode.textContent.trim() : '',
        price: priceNode ? priceNode.getAttribute('data-price') : '',
        href: card.getAttribute('href') || 'availability.html'
      };
      nameEl.textContent = current.name;
      locEl.textContent = current.loc;
      render();
      lastFocus = document.activeElement;
      modal.hidden = false;
      requestAnimationFrame(function () { modal.classList.add('is-open'); });
      document.body.classList.add('is-locked');
      $('#qmodalClose').focus({ preventScroll: true });
    }
    ...
    /* arrow triggers — stop the card's own navigation */
    $$('.feat-card__go').forEach(function (go) {
      var card = go.closest('.feat-card');
      on(go, 'click', function (e) {
        e.preventDefault();
        e.stopPropagation();
        open(card);
      });
      on(go, 'keydown', function (e) {
    ...
    /* form submit → unlock everywhere, reveal here with a count-up */
    on(modal, 'submit', function (e) {
      e.preventDefault();
      var nm = $('#qf-name', modal), em = $('#qf-email', modal);
      var ok = true;
      [nm, em].forEach(function (i) { i.parentElement.classList.remove('is-error'); });
      if (!nm.value.trim()) { nm.parentElement.classList.add('is-error'); ok = false; }
      ...
      try {
        ...
        leads.push({ unit: current.name, name: nm.value.trim(), email: em.value.trim(), phone: '' });
        localStorage.setItem('swamiLeads', JSON.stringify(leads));
        localStorage.setItem('swamiPriceUnlocked', '1');
      } catch (err) { /* private mode — still reveal for this view */ }
      ...
      setTimeout(function () {
        render();
        var v = $('#qVal');
        if (v && current.price) fmtAnimate(v, current.price);
        if (revealPrices) { revealPrices(); revealPrices = null; }   // card prices too
      }, 600);
    });
    on(modal, 'input', function (e) {
      var f = e.target.closest('.field');
      if (f) f.classList.remove('is-error');
    });

    on($('#qmodalClose'), 'click', close);
  ...
  function boot() {
    ...
    safe('quickPrice', initQuickPrice);
    safe('misc', initMisc);
